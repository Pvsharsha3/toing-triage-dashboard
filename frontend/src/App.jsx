...
// Local: calls backend at localhost:8000 (externalbrowser SSO, like SAGE)
// GitHub Pages: VITE_API_URL points to Render backend
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000'
const HAS_BACKEND = !!import.meta.env.VITE_API_URL || !import.meta.env.PROD
...
async function fetchOrder(orderId) {
  ...
  // Demo mode — local mock data
  const data = MOCK_ORDERS[orderId.trim()]
  if (!data) throw new Error(`Order not in demo set. Try: ${TEST_ORDER_IDS.map(t => t.id).join(', ')}`)
  return data
}
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="ml-auto">
            <span className={`text-xs rounded px-2 py-1 ${HAS_BACKEND ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
              {HAS_BACKEND ? '🟢 Live — Snowflake' : '🟡 Demo mode'}
            </span>
          </div>
        </div>
      </header>
            ...
            {data.status === 'cancelled' && (
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 mt-0.5">Discount config shown is what partners see — order was cancelled before funds transferred.</p>
                </div>
              </div>
            )}
